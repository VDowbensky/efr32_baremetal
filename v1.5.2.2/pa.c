...
{
  if ((apcEnabled == '\0') || (param_1 < 0x79)) BUS_RegMaskedClear(&RAC->SR3,8);
  else 
  {
...
void PA_PowerOffsetSet(uint16_t offset)
...

{
  ...
  uVar1 = (uint)pwrLevel;
  if (gPaConfig.paSel == PA_SEL_INVALID) return 0;
  if (gPaConfig.paSel != PA_SEL_2P4_LP) 
  {
	SEQ->REG0E4 &= 0xe0c03fff;
	SEQ->REG0E4 |= 0x3fc8;;
    iVar4 = count_leading_zeroes(~CMU->RFLOCK0 & 0x1fc000);
    ...
    SEQ->REG0E4 = uVar3 | (uVar5 % 0x1f) * 0x1000000 + 0x1000000 |((1 << uVar5 / 0x1f + 1) - 1U & 0xff) << 0xe;
    if ((uVar2 == 0) || (gPaConfig.paSel != PA_SEL_2P4_HP)) BUS_RegMaskedClear(&RAC->PABIASCTRL0,0x400000);
    else 
	{
      uVar1 = uVar1 | 0x100;
...

{
  ...
  if (gPaConfig.paSel == PA_SEL_INVALID) return;
  uVar1 = SEQ->REG0E4;
  if (param_1 < 0x83) 
  {
...

{
  ...
  if (gPaConfig.paSel != PA_SEL_INVALID) 
  {
    ...
    else 
	{
      ...
      iVar1 = uVar3 * 0x44;
      uVar2 = PA_StripesAndSlicesCommonCalc(power + gPaConfig.offset,(uint16_t *)(&paParams + iVar1));
      uVar2 = PA_StripesAndSlicesSet(uVar2);
      if (uVar2 == 0x21) uVar2 = SEXT24(*(short *)(&DAT_0001b55a + iVar1));
      else uVar2 = PA_PowerFromStripesAndSlicesCommonCalc(uVar2,(uint32_t)(&UNK_0001b55c + iVar1));
    }
...

{
  ...
  if ((uVar3 < 5) && ((1 << uVar3 & 0x13U) != 0)) uVar3 = 0;
  else 
  {
    if (gPaConfig.paSel == PA_SEL_SUBGIG) uVar3 = 0x96;
    else 
	{
      if (gPaConfig.paSel == PA_SEL_2P4_LP) uVar3 = 0x32;
      else uVar3 = 0xb4;
    }
  }
  ...
  cVar2 = -1;
  for (uVar4 = (((uVar1 / 1000) * ramptime) / uVar3 << 5) / 1000; uVar4 != 0; uVar4 = uVar4 >> 1) cVar2 = cVar2 + '\x01';
  iVar5 = (int)cVar2;
  if (iVar5 < 0x10) 
  {
    if (iVar5 < 0) iVar5 = 0;
...

{
  ...
  if (paInit == (RADIO_PAInit_t *)0x0) return false;
  RADIO_CLKEnable();
...
{
  PA_BandSelect();
  if ((uint)gCurrentModulation != (MODEM->CTRL0 << 0x17) >> 0x1d) 
  {
