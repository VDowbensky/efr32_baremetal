...

{
  ...
  do {
    ...
    if ((FRC *)((uint)puVar1 & 0xffff0000) == &Peripherals::FRC) {
...
switchD_00010218_caseD_8:
  while( true ) {
    if ((param_2 == (uint *)0x0) || (uVar2 = *param_2, uVar2 == 0xffffffff)) {
      PA_UpdateConfig(param_1);
      TIMING_SeqTimingInit();
      return;
    }
    uVar1 = (uVar2 << 8) >> 0x18;
    puVar4 = (uint *)(uVar2 & 0xffff | *(uint *)(&regBases + ((uVar2 << 4) >> 0x1c) * 4));
    uVar3 = uVar1;
    puVar5 = param_2;
    if (uVar1 < 2) break;
    do {
      uVar3 = uVar3 - 1;
      *(uint *)(((int)puVar4 - (int)param_2) + (int)puVar5) = puVar5[1];
      puVar5 = puVar5 + 1;
    } while (uVar3 != 0);
...
{
  write_volatile_4(Peripherals::RAC_SET.SR3,0x80);
  return;
}
...
{
  write_volatile_4(Peripherals::RAC_CLR.SR3,0x80);
  return;
}
...

{
  ...
  do {
    uVar4 = uVar2 + 1;
    if (uVar3 <= (uint)(1 << (uVar4 & 0xff))) goto LAB_00010642;
    uVar2 = uVar4;
  } while (uVar4 != 0x10);
  uVar2 = 0xf;
LAB_00010642:
...

{
  ...
  return (int)((ulonglong)(uint)(iVar4 * 100000) / ((ulonglong)uVar3 / 10));
}
