...
LAB_000101c0:
  ...
  if ((param_1 & 0x8000) == 0) 
  {
		BUS_RegMaskedClear(&RAC->PABIASCTRL0 , RAC_PABIASCTRL0_TXPOWER_Msk); //TXPOWER off
    return uVar1;
  }
...
//  pwr = pwr - 1 & 0xff;
	...
	if (pwr >= 0xf9) pwr = 0xf8;
  if (pwr != 0) pwr = (pwr - 1) & 0xff;
//LAB_00010240:
...

{
  ...
  else
  {
    ...
    iVar2 = uVar3 * 0x44;
    //PA_StripesAndSlicesCommonCalc(pwr + gPaConfig.offs, &paParams + iVar2,&paParams,uVar3,param_4);
...
{
  if (!(SYNTH_Is2p4GHz())) RAC->SGPACTRL0 &= 0x7fdfffff; //31, 17 CASCODEBYPASSEN, SLICE
  else RAC->PACTRL0 &= 0x7fdfffff; //
