...
bool Si7210_Stop(void)
{
  ...
  //wake up the si7200
  ret = Si72xx_WakeUp(I2C, si7210adr);
  ret |= Si72xx_Write_Register(I2C, si7210adr, SI72XX_CTRL3, 0x00);
  ret |= Si72xx_Write_Register(I2C, si7210adr, SI72XX_POWER_CTRL, USESTORE_MASK|STOP_MASK|SLEEP_MASK);
...
bool Si7210_Data_(int16_t *value)
{
  ...
  ret = Si72xx_WakeUp(I2C, si7210adr);
  ret |= Si72xx_Write_Register(I2C, si7210adr, SI72XX_POWER_CTRL, ONEBURST_MASK);
  //ret |= Si72xx_Read_Data(i2c, i2cAddr, value);

  ret |= Si72xx_Read_Register(I2C,si7210adr,SI72XX_DSPSIGM,&read);
  flag = read>>7;
  *value = (((uint16_t)read)&0x7f)<<8;
  ret |= Si72xx_Read_Register(I2C,si7210adr,SI72XX_DSPSIGL,&read);
  *value |= read;
  *value = *value - 16384;

  ret |= Si72xx_Sleep(I2C, si7210adr);
...
bool Si7210_Set_sw_op(uint8_t value)
{
  ...
  ret |= Si72xx_Write_Register(I2C, si7210adr, SI72XX_CTRL1, value);
  ret |= Si72xx_Sleep(I2C, si7210adr);
...
bool Si7210_Set_sw_hyst(uint8_t value)
{
  ...
  ret = Si72xx_WakeUp(I2C, si7210adr);
  ret |= Si72xx_Read_Register(I2C,si7210adr,SI72XX_CTRL2,&read);
  read &= 0xC0;
  read |= (value&0x3F);
  ret |= Si72xx_Write_Register(I2C, si7210adr, SI72XX_CTRL2, read);
  ret |= Si72xx_Sleep(I2C, si7210adr);
...
bool Si7210_Get_sw_op(uint8_t * value)
{
  ...
  ret = Si72xx_WakeUp(I2C, si7210adr);
  ret |= Si72xx_Read_Register(I2C,si7210adr,SI72XX_CTRL1,&read);
  ret |= Si72xx_Sleep(I2C, si7210adr);
  *value = read&0x7F;
...
bool Si7210_SensorDataBuf(uint8_t *buf)//debug
{
  ...
  for (uint8_t a = 0xC0, i=0; a <= 0xD0; ++a, ++i) {
    ret |= Si72xx_Read_Register(I2C, si7210adr, a, &buf[i]);
  }
  ret |= Si72xx_Sleep(I2C, si7210adr);
  if (ret != i2cTransferDone) {
		//printf("I2C error. ret =%d", ret);
    return false;
  }
	//printf("I2C OK. ret =%d", ret);
  return true;
}
...
bool Si7210_Detect(I2C_TypeDef *i2c, uint8_t *addr)
{
  ...
  if (si7210adr == 0) {
		//printf("I2C error0. dbg =%d\r\n", dbg);
    return false;
  }
  if (ret != i2cTransferDone) {
		//printf("I2C error1. ret =%d", ret);
    return false;
  }
  //ReadField();
...
void ReadField(void)
{
  FieldValue = Si72xx_ReadFieldValue(I2C, si7210adr, 0);
  if (si7210phase == 0)
    {
